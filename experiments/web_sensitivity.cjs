...
function run(seed, parameter, delta=0){
  ...
  if(parameter) { const input=e.state.controls.get(parameter).input;input.value=String(Number(input.value)+delta); }
...
const report={purpose:'Diagnóstico local exploratorio, no validación empírica ni sensibilidad global',
  agents:80,rounds:120,seeds,baseline,results};
const output=path.join(__dirname,'../docs/web_sensitivity_results.json');
...
console.log('78 ejecuciones; resultados y semillas en docs/web_sensitivity_results.json');
